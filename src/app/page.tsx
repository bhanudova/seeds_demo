"use client"
import Image from "next/image";
import Link from "next/link";
import { useEffect, useState } from "react";
import { AiOutlineClose, AiOutlineMenu } from "react-icons/ai";

export default function Navbar({ theme }: any) {

  const [nav, setNav] = useState(false)
  const handelNav = () => {
    setNav(!nav)
  }



  return (
    <div>
      <div className="fixed w-screen bg-white px-6 z-10">
        <div className="flex justify-between items-center py-4 md:py-2 text-accent-content">

          <div className="hidden lg:block">
            <Image src={'/logo.png'} width={500} height={500} alt="logo" className="w-28" />
          </div>

          <div className="hidden lg:block">
            <div className="flex gap-2 ">
              <ul className="flex gap-4 font-bold">
                <li>Home</li>
                <li>Fertilizer</li>
                <li>Seeds</li>
                <li>Foods</li>
                <li>Tissue Culture</li>
              </ul>
              <label className="swap swap-rotate mt-[-4px]">
                <input type="checkbox" />
                <svg className="swap-on fill-current w-7 h-7" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M5.64,17l-.71.71a1,1,0,0,0,0,1.41,1,1,0,0,0,1.41,0l.71-.71A1,1,0,0,0,5.64,17ZM5,12a1,1,0,0,0-1-1H3a1,1,0,0,0,0,2H4A1,1,0,0,0,5,12Zm7-7a1,1,0,0,0,1-1V3a1,1,0,0,0-2,0V4A1,1,0,0,0,12,5ZM5.64,7.05a1,1,0,0,0,.7.29,1,1,0,0,0,.71-.29,1,1,0,0,0,0-1.41l-.71-.71A1,1,0,0,0,4.93,6.34Zm12,.29a1,1,0,0,0,.7-.29l.71-.71a1,1,0,1,0-1.41-1.41L17,5.64a1,1,0,0,0,0,1.41A1,1,0,0,0,17.66,7.34ZM21,11H20a1,1,0,0,0,0,2h1a1,1,0,0,0,0-2Zm-9,8a1,1,0,0,0-1,1v1a1,1,0,0,0,2,0V20A1,1,0,0,0,12,19ZM18.36,17A1,1,0,0,0,17,18.36l.71.71a1,1,0,0,0,1.41,0,1,1,0,0,0,0-1.41ZM12,6.5A5.5,5.5,0,1,0,17.5,12,5.51,5.51,0,0,0,12,6.5Zm0,9A3.5,3.5,0,1,1,15.5,12,3.5,3.5,0,0,1,12,15.5Z" /></svg>
                <svg className="swap-off fill-current w-7 h-7" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M21.64,13a1,1,0,0,0-1.05-.14,8.05,8.05,0,0,1-3.37.73A8.15,8.15,0,0,1,9.08,5.49a8.59,8.59,0,0,1,.25-2A1,1,0,0,0,8,2.36,10.14,10.14,0,1,0,22,14.05,1,1,0,0,0,21.64,13Zm-9.5,6.69A8.14,8.14,0,0,1,7.08,5.22v.27A10.15,10.15,0,0,0,17.22,15.63a9.79,9.79,0,0,0,2.1-.22A8.11,8.11,0,0,1,12.14,19.73Z" /></svg>
              </label>
            </div>
          </div>

        </div>
      </div>



      <div className="main-image">
        <div className="absolute top-0 left-0 w-full h-full bg-black opacity-5"></div>
<div className="absolute top-0 left-0 w-full h-full bg-black opacity-50"></div>
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-center text-white  uppercase">
          <h1 className="text-4xl font-bold"><span>Welcome To Prasad Seeds</span></h1>
          <p className="font-medium">
            Prasad Seeds Pvt. Ltd. (PSPL) is India’s largest revolutionizing agricultural industry with technologies and innovative solutions.
          </p>
        </div>
      </div>


    </div>
  )
}